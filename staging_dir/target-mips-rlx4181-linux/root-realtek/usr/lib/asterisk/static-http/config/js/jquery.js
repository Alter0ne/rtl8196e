...
jQuery.extend({
	...
	ajax: function( s ) {
		...
		if ( s.global && ! jQuery.active++ ){
			if( !s.url.contains('rawman?action=ping') ){	// ASTGUI modification -- do not trigger the event for GUI ping requests
	...
	handleError: function( s, xhr, status, e ) {
		try{ // modification for ASTGUI -- return 404 to the ASTGUI's callback function so it knows that the script failed to load
	...
	param: function( a ) {
		...
		return s.join("&");
		//return s.join("&").replace(/%20/g, "+"); // ASTGUI modification - making asterisk webserver compliant
	}
